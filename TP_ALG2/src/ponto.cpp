...
Triangulo Ponto::menorTri(vector<Ponto> &vetor, size_t l, size_t r) {
  if(r - l + 1 <= 10){
    Triangulo best;
    best.x = -1; best.y = -1; best.z = -1; best.perimetro = -1;
    double p = -1;
    for (size_t a = l; a <= r; ++a) {
        for (size_t b = a + 1; b <= r; ++b) {
            for (size_t c = b + 1; c <= r; ++c) {
                ...
                if (peri < p || p == -1) {
                    p = peri;
                    best.x = id1; best.y = id2; best.z = id3; best.perimetro = peri;
                } else if (peri == p) {
                    if (best.x == -1
                        || id1 < best.x
                        || (id1 == best.x && id2 < best.y)
                        || (id1 == best.x && id2 == best.y && id3 < best.z)) {
                        best.x = id1; best.y = id2; best.z = id3; best.perimetro = peri;
                    }
                }
            }
  ...
  if (p == esquerda.perimetro && p == direita.perimetro) {
    if (esquerda.x > direita.x) { menor = direita;
    } else if (esquerda.x == direita.x && esquerda.y > direita.y) { menor = direita;
    } else if (esquerda.x == direita.x && esquerda.y == direita.y && esquerda.z > direita.z) { menor = direita;
    } else { menor = esquerda;
    }
      ...
      for (size_t k = j + 1; k < strip.size(); k++) {
        ...
        if (peri <= p) {
          ...
          if (peri == p) {
            ...
            if (menor.x > id1) { menor = meio;
            } else if (menor.x == id1 && menor.y > id2) { menor = meio;
            } else if (menor.x == id1 && menor.y == id2 && menor.z > id3) { menor = meio;
            }
          } else {
            menor = meio;
            p = peri;
